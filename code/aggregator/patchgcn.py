...
class DeepGraphConv_Surv(torch.nn.Module):
    ...
    def relocate(self):
        ...
        if torch.cuda.device_count() >= 1:
            ...
            self.path_attention_head = nn.DataParallel(
                self.path_attention_head, device_ids=device_ids
            ).to("cuda:0")
    ...
    def forward(self, feat, edge_index, edge_latent):
        ...
        if self.pool:
            x1, edge_index, _, batch, perm, score = self.pool1(
        ...
        if self.pool:
            x2, edge_index, _, batch, perm, score = self.pool2(
                x2, edge_index, None, batch
            )
...
class PatchGCN_Surv(torch.nn.Module):
    def __init__(
        ...
        if self.resample > 0:
            ...
            self.fc = nn.Sequential(
                *[nn.Linear(ndim, 128), nn.ReLU(), nn.Dropout(0.25)]
            )
