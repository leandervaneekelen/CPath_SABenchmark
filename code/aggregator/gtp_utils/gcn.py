...
class AGCNBlock(nn.Module):
    ...
    def forward(self, X, adj, mask, is_print=False):
        ...
        if self.model == "unet":
            ...
            if self.softmax == "global" or self.softmax == "mix":
                if False:
                    dgree_w = (
                        torch.sum(adj, dim=2)
                        / torch.sum(adj, dim=2).max(1, keepdim=True)[0]
                    )
                    att_a = (
                        torch.matmul(hidden, self.w_a).squeeze() * dgree_w
                        + (mask - 1) * 1e10
                    )
                else:
            ...
            if self.softmax == "neibor" or self.softmax == "mix":
                ...
                if self.tau_config != -2:
                    ...
                    att_b = torch.exp(
                        (att_b - att_b_max)
                        * torch.abs(self.tau_fc(self.pool(hidden, mask)))
                    )
        ...
        k_list = [
            int(math.ceil(self.filt_percent * x)) for x in mask.sum(dim=1).tolist()
        ]
        # print('k_list')
        # print(k_list)
        if self.model != "diffpool":
            if self.sort == "sample":
                ...
                for i in range(att.size(0)):
                    top_index = (
                        torch.LongTensor(
                            np.random.choice(att_samp.size(1), k_max, att_samp[i])
                        ),
                    )
                ...
                for i in range(att.size(0)):
                    top_index[i, 0 : k_list[i]] = torch.randperm(
                        int(mask[i].sum().item())
                    )[0 : k_list[i]]
            else:  # sort
        ...
        if self.model == "unet":
            ...
            # print('H')
            # print(H.shape)
            new_adj = torch.matmul(
                torch.matmul(assign_m, adj), torch.transpose(assign_m, 1, 2)
            )
            ...
            assign_m = X.new_ones(X.shape[0], X.shape[1], k_max) * (-100000000.0)
        ...
        if self.att_out and self.model == "agcn":
            if self.softmax == "global":
                ...
                out = self.pool(
                    (att_b / (att_b_sum + self.eps)).unsqueeze(2) * hidden, mask
                )
        else:
            # print('hidden.shape')
            # print(hidden.shape)
...
class GCNBlock(nn.Module):
    ...
    def forward(self, x, adj, mask):
        ...
        if self.bn:
            ...
            for i in range(x.shape[0]):
                bn_tensor_af[i, 0 : index[i]] = bn_tensor_bf[
