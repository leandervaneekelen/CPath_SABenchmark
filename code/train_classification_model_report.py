...
parser.add_argument(
    ...
    type=str,
    default="",
    choices=["tres50_imagenet", "dinosmall", "dinobase"],
...
def main(config=None):
    ...
    model = modules.get_aggregator(
        method=args.method, ndim=args.ndim
    )  # model = modules.get_aggregator(method='AB-MIL', ndim=256)
    ...
    for epoch in range(args.nepochs + 1):
        if epoch in [
            ...
            30,
            39,
        ]:  # Special case for testing feature extractor
    ...
    if args.method in ["PatchGCN", "DeepGraphConv"]:
        ...
        edge_index = torch.randint(
            0, 256, (2, 256), dtype=torch.int64
        ).cuda()  # Random edge indices
        edge_latent = torch.randint(
            0, 256, (2, 256), dtype=torch.int64
        ).cuda()  # Random edge indices for latent graph
        model_summary = summary(
            model, input_data=(feat, edge_index, edge_latent), verbose=0
        )
    else:
    ...
    wandb.run.summary["model_size (MB)"] = calculate_model_size(model) / (
        8 * 1024 * 1024
    )
    ...
    with torch.no_grad():
        for i, input in enumerate(loader):  #
            ...
            torch.cuda.reset_peak_memory_stats(
                device
            )  # Reset memory stats for the next step
...
def train(run, loader, model, criterion, optimizer, lr_schedule, wd_schedule, device):
    ...
    for i, input in enumerate(loader):  #
        ...
        torch.cuda.reset_peak_memory_stats(
            device
        )  # Ensure to reset for the next measurement
...
def update_args_with_best_hyperparameters(args):
    ...
    best_hyperparameters = find_best_hyperparameters(
        ...
        args.method,
        metric="val_auc",
        config_interest=["lr", "weight_decay"],
    )
