...
def run(model, data, device, use_amp=True):
    ...
    model_summary = summary(
        model, input_size=(data.shape[0], data.shape[1], data.shape[2]), verbose=0
    )
    ...
    if device.type == "cuda":
        ...
        memory_gpu = None
    memory_cpu = (
        sum(p.numel() for p in model.parameters() if not p.is_cuda)
        * data.element_size()
    )
...
def suggest_memory_requirements(memory_usage_data):
    ...
    suggested_gpu = min(
        (gpu for gpu, mem in thresholds.items() if mem >= max_memory_gpu),
